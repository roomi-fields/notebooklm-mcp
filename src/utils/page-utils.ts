 ...
 * We only strip them when they appear as isolated lines to avoid mutating
 ...
 *
 * NotebookLM sometimes exposes citation/menu button labels in innerText, e.g.:
 *   "Answer text\n1\nmore_horiz\n."
 *
 * We strip isolated icon-label lines and any adjacent citation-number/punctuation
 * lines that only exist because the UI controls were included in the text dump.
 */
export function sanitizeResponseText(text: string): string {
  ...
  for (let i = 0; i < rawLines.length; i++) {
    ...
    // Citation buttons can leak as a standalone number right before a menu icon label.
    if (/^\d+$/.test(line) && nextIsUi) {
      continue;
    }

    // Menu icons can also leave behind isolated punctuation after stripping.
